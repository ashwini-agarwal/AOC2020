...
function countAdjacentOccupiedSeats(layout, x, y) {
  let occupiedSeates = 0;
  for (let i = -1; i <= 1; i++) {
    for (let j = -1; j <= 1; j++) {
      if (i === 0 && j === 0) {
        continue;
      }
      occupiedSeates = layout[y + i] && layout[y + i][x + j] && layout[y + i][x + j] === '#' ? occupiedSeates + 1 : occupiedSeates;
    }
  }
...
let initialLayout = layout.map((row) => row.split(''));
let stabilized;
let trial = 0;
...
do {
  ...
  initialLayout.forEach((rowLayout, y) => {
    if (!updatedLayout[y]) {
      updatedLayout[y] = [];
    }

    rowLayout.forEach((place, x) => {
      updatedLayout[y][x] = initialLayout[y][x];
      const adjacentOccupiedSeats = countAdjacentOccupiedSeats(initialLayout, x, y);
      switch (place) {
        case 'L':
...
const occupiedSeates = initialLayout.reduce((rowCount, row) => rowCount + row.reduce((count, place) => place === '#' ? count + 1 : count, 0), 0);
console.log(occupiedSeates);
