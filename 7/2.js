...
rules.forEach(element => {
  ...
  innerBagsArray.forEach(([count, bag]) => {
    if (!bag) {
      return;
    }

    if (!parsed_rules[outer]) {
      parsed_rules[outer] = [];
    }
    parsed_rules[outer].push({ bag: bag.trim(), count: parseInt(count) });
  });
});

function findMaxChilrenBags(key, counter = 0) {
  if (!parsed_rules[key]) {
    return 0;
  }

  parsed_rules[key].forEach(({ bag, count }) => {
    counter = counter + count + count * findMaxChilrenBags(bag);
  });
  return counter;
}

console.log(findMaxChilrenBags('shiny gold'));
