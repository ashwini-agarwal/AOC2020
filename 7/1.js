...
rules.forEach(element => {
  ...
  innerBagsArray.forEach(([count, bag]) => {
    ...
    parsed_rules[outer].push({ bag: bag.trim(), count });
  });
});

let counter = 0;
let covered = [];
function findAllContainer(search) {
  Object.keys(parsed_rules).forEach(key => {
    if (covered.includes(key)) {
      return counter;
    }
    const containers = parsed_rules[key].filter(({ bag, count }) => bag === search);
    if (containers.length) {
      counter += 1;
      covered.push(key);
      return findAllContainer(key);
    }
  });
  return counter;
}

console.log(findAllContainer('shiny gold'));